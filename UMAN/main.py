...
class TotalNet(nn.Module):
    def __init__(self):
        ...
        classifier_output_dim = source_classes
...
if args.test.test_only:
    ...
    counters = [AccuracyCounter() for x in range(source_classes + 1)]
    with TrainingModeManager([feature_extractor, classifier], train=False) as mgr, \
            ...
            torch.no_grad():
        for i, (img1) in enumerate(tqdm(target_test_dl, desc='testing ')):
            im = img1['X']
            label = img1['Y']
            im = im.cuda()
            label = label.cuda()
    ...
    with TrainingModeManager([feature_extractor, classifier], train=False) as mgr, \
            ...
            torch.no_grad():
        for i, (img1) in enumerate(tqdm(source1_test_dl, desc='testing ')):

            im_source1 = img1['X']
            label_source1 = img1['Y']
    ...
    with TrainingModeManager([feature_extractor, classifier], train=False) as mgr, \
            ...
            torch.no_grad():
        for i, (img1) in enumerate(tqdm(source2_test_dl, desc='testing ')):

            im_source2 = img1['X']
            label_source2 = img1['Y']
...
epoch_id = 0
class_temperture = torch.zeros(source_classes, 1).cuda()
...
while global_step < args.train.min_step:
    ...
    for i, (img1, img2, imgt) in enumerate(iters):
        
        im_source1 = img1['X']
        label_source1 = img1['Y']

        im_source2 = img2['X']
        label_source2 = img2['Y']
        ...
        with torch.no_grad():
            ...
            target_accumulated_margin = torch.zeros(source_classes, 1).cuda()
            target_pseudo_label = predict_prob_target.max(1)[1]
            num_per_class = torch.zeros(source_classes, 1).cuda()
            ...
            counter.addOneBatch(variable_to_numpy(torch.nn.functional.one_hot(label_source1, source_classes)), variable_to_numpy(predict_prob_source1))
            counter.addOneBatch(variable_to_numpy(torch.nn.functional.one_hot(label_source2, source_classes)), variable_to_numpy(predict_prob_source2))
