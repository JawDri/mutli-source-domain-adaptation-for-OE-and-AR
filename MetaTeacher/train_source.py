...
model_names = sorted(name for name in resnet20.__dict__
    if not name.startswith("__")
                     and name.startswith("ResNet")
                     and callable(resnet20.__dict__[name]))
...
Source_test = pd.read_csv("./data/Source_test_1.csv")
Source_train = pd.read_csv("./data/Source_train_1.csv")
...
def main(config):
    ...
    model = torch.nn.DataParallel(resnet20.__dict__[args.arch]())
    model.cuda()
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #print(output[4])
        loss = criterion(output[4], target_var)
        ...
        #output[4] = output[4].float()
        loss = loss.float()
        # measure accuracy and record loss
        prec1 = accuracy(output[4].float(), target)[0]
...
if __name__ == '__main__':
    config = ['--arch', 'ResNet32', '--epochs', '20']
    main(config)