...
class Solver(object):
    ...
    def train(self, epoch, record_file=None):
        ...
        for batch_idx, data in enumerate(self.datasets):
            ...
            img_t = img_t.view(-1,1, 9)#32/9
            img_s1 = img_s1.view(-1,1, 9)
            img_s2 = img_s2.view(-1,1, 9)
            img_s3 = img_s3.view(-1,1, 9)
            img_s4 = img_s4.view(-1,1, 9)
    ...
    def test(self, epoch, record_file=None, save_model=False):
        ...
        for batch_idx, data in enumerate(self.dataset_test):
            ...
            fscore += f1_score(label.data.cpu(), pred1.cpu(), average='weighted')
            fcount+=1
            size += k
        np.savez('result_plot_sv_t', feature_all, label_all )
        test_loss = test_loss / size
        print(
            '\nTest set: Average loss: {:.4f}, Accuracy C1: {}/{} ({:.2f}%) F1score: ({:.2f}%) \n'.format(
                test_loss, correct1, size,   100. * correct1 / size,  100. * fscore / fcount))
